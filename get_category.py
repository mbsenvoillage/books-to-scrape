...
async def cat_page_url(url, starturl, queue: asyncio.Queue, semaphore):
    """Takes the URL of a category page and returns the URL of the next category page, if there is one"""
    soup = await get_soup(urlparse(url).geturl(), semaphore)
...
async def producer(outgoing_queue: asyncio.Queue, incoming_queue: asyncio.Queue, semaphore):
...
async def scrape(url: str, number_of_books, semaphore):
    ...
    list_of_urls = []
    tasks = []
    length = how_many_pages(await get_soup(url, semaphore))
    await url_queue.put(url)
    if (length > 1):
        for i in range(length):
            task = asyncio.create_task(cat_page_url(reformat_cat_page_url(url, i+1), url, url_queue, semaphore))
            tasks.append(task) 
    tasks.extend(asyncio.create_task(producer(soup_queue, url_queue, semaphore)) for _ in range(number_of_books))
